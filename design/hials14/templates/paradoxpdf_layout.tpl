...
Module_result.title_path:
{$module_result.title_path[3]|attribute( 'show', 2, 'text' )}
...
<body>
{if eq($module_result.content_info.class_identifier, 'studiehandbok')}
<div id="titlepage">
	<p align="center" style=">{$site.title}</p>
</div>
{/if}
<div id="header">
    <div id="sitetitle">{$site.title}</div>
    <div id="line2">
      <div id="edition">{'Edition'|i18n('hials/design/shb')}: {$current_shb_name|wash()}</div>
      <div id="article">Content name</div>
      <div id="date">{'PDF generated'|i18n('hials/design/std')}: {currentdate()|datetime( 'custom', '%r' )}</div>
    </div>
</div>
<div id="footer">
	<p>{"Page"|i18n('hials/design/std')} <span id="pagenumber"/> / <span id="pagecount"/></p>
</div>
{include uri="design:shb/parts/shb_pdf_toc.tpl"}
