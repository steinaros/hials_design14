				        ...
				        <ul class="dropdown-menu">
				            ...
				            <li>{foreach $level2_items as $level2key => $level2item}
				                <div class="{if $level2key|eq(0)}col-sm-offset-2 col-sm-2{else}col-sm-2{/if}">
				                    <ul class="submenu">
				                        <li class="submenuhead"><a href={if eq( $ui_context, 'browse' )}{concat("content/browse/", $level2item.node_id)|ezurl}{else}{$level2item.url_alias|ezurl}{/if}>{cond($level2item.data_map.menutitle.$item.data_map.menutitle.has_content, $level2item.data_map.menutitle.content|wash(), $level2item.name|wash())}{/if}</a></li>
				                    {set $level3_items = fetch( 'content', 'list', hash( 'parent_node_id', $level2item.node_id,
					                                              'sort_by', $level2item.sort_array,
					                                              'class_filter_type', 'include',
					                                              'class_filter_array', $top_menu_class_filter ) )
		                            ...
		                            {if $level3_items_count}
		                                {foreach $level3_items as $level3key => $level3item}
		                                    <li><a href={if eq( $ui_context, 'browse' )}{concat("content/browse/", $level3item.node_id)|ezurl}{else}{$level3item.url_alias|ezurl}{/if}>{cond($level3item.data_map.menutitle.has_content, $level3item.data_map.menutitle.content|wash(), $level3item.name|wash())}{/if}</a></li>
		                                {/foreach}
		                            {/if}
		                            </ul>
		                        </div>
