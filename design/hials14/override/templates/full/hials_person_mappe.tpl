...
<div class="hidden-xs">
    ...
    {foreach $menu_tree as $item}
      <li role="presentation"{if eq($item.node_id, $node.node_id)} class="active"{/if}><a href="{$item.url_alias|ezurl( 'no' )}"><span class="text">{'Employees at'|i18n('hials/design/person')} {cond(is_object($node.data_map.menutitle), $node.data_map.menutitle.content|wash()|downcase(), $node.data_map.title.content|wash() )}</span><span class="glyphicon glyphicon-chevron-right"></span></a></li>
    {/foreach}
    </ul>           
</div>
<div class="content-view-full row">
    <article class="class-hials_person_mappe col-sm-12">
        <h1>{cond(is_object($node.data_map.menutitle), $node.data_map.menutitle.content|wash(), $node.data_map.title.content|wash()}</h1>
