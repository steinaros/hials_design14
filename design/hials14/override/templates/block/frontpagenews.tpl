...
<div class="block-type-content-grid block-view-{$block.view} row-wrapper">
    ...
    <div class="col-sm-6 col-top" itemscope itemtype="http://schema.org/Article">
        <article class="front headline">
            <span class="glyphicon glyphicon-chevron-left hidden-xs" style="position: absolute;font-size: 28px;color: #f1f1f1;margin-top: 35px;left:-15px;"></span>
            ...
            <div class="teaser-intro"><time itemprop="datePublished" datetime="{$valid_nodes.0.object.published|datetime('custom', '%c')}">{$valid_nodes.0.object.published|l10n('shortdate')}</time><span itemprop="description">{attribute_view_gui attribute=$valid_nodes.0.data_map.intro}</span></div>
        </article>
        <div class="row">
            ...
            <article class="col-sm-6" itemscope itemtype="http://schema.org/Article">
                ...
                <div class="teaser-intro"><time itemprop="datePublished" datetime="{$item.object.published|datetime('custom', '%c')}">{$item.object.published|l10n('shortdate')}</time><span itemprop="description">{attribute_view_gui attribute=$item.data_map.intro}</span></div>
            </article>
            {/foreach}
        </div>
